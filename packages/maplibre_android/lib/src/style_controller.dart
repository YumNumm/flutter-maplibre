...
class StyleControllerAndroid extends StyleController {
  ...
  Future<void> updateFilter({
    ...
    if (jLayer.isA(jni.FillLayer.type)) {
      jLayer.as(jni.FillLayer.type).filter = effectiveFilter;
    } else if (jLayer.isA(jni.CircleLayer.type)) {
      jLayer.as(jni.CircleLayer.type).filter = effectiveFilter;
    } else if (jLayer.isA(jni.FillExtrusionLayer.type)) {
      jLayer.as(jni.FillExtrusionLayer.type).filter = effectiveFilter;
    } else if (jLayer.isA(jni.HeatmapLayer.type)) {
      jLayer.as(jni.HeatmapLayer.type).filter = effectiveFilter;
    } else if (jLayer.isA(jni.LineLayer.type)) {
      jLayer.as(jni.LineLayer.type).filter = effectiveFilter;
    } else if (jLayer.isA(jni.SymbolLayer.type)) {
      jLayer.as(jni.SymbolLayer.type).filter = effectiveFilter;
    } else {
